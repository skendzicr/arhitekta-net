...
export class MetaDataService {
  ...
  createMetaData(description = DEFAULT_META_DESCRIPTION, title = DEFAULT_META_TITLE, url = DEFAULT_META_URL, image = DEFAULT_META_IMAGE) {
    title = `${title} ${SITE_NAME}`;
    this.titleService.setTitle(title);
    this.meta.updateTag({ property: 'description', content: description }, 'property=\'description\'');
